...
-- [SELECT] All testimonies with customer name and package
SELECT
    t.testimony_id,
    CONCAT(u.first_name, ' ', u.last_name) AS customer,
    p.package_name,
...
JOIN User     u ON b.user_id     = u.user_id
JOIN Package  p ON b.package_id  = p.package_id
ORDER BY t.testimony_date DESC;

-- [SELECT] Top-rated testimonies (for public display / marketing)
SELECT
    CONCAT(u.first_name, ' ', u.last_name) AS customer,
    p.package_name,
    t.rating,
...
JOIN Package  p ON b.package_id  = p.package_id
WHERE t.rating >= 4
ORDER BY t.rating DESC, t.testimony_date DESC;

-- [SELECT] Average rating per package
SELECT
    p.package_name,
    ROUND(AVG(t.rating), 2) AS avg_rating,
    COUNT(t.testimony_id)   AS total_reviews
FROM Testimony t
JOIN Booking  b ON t.booking_id  = b.booking_id
JOIN Package  p ON b.package_id  = p.package_id
...
-- [SELECT] Check if a booking already has a testimony
SELECT testimony_id FROM Testimony WHERE booking_id = 1;

-- [INSERT] Submit a testimony (only after event is Completed)
INSERT INTO Testimony (booking_id, message, rating, testimony_date)
VALUES (3, 'Everything was perfect! Will definitely book again.', 5, CURDATE());

-- [UPDATE] Edit testimony message or rating
UPDATE Testimony
SET message = 'Very good service. Slight delay at start but overall great!',
    rating  = 4
WHERE testimony_id = 2;

-- [DELETE] Remove a testimony
DELETE FROM Testimony WHERE testimony_id = 3;
